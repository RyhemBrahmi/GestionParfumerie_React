...
class UpdateHairCare extends Component {
    constructor(props){
        super(props)
        this.state = {
        ...
        this.updateHairCare = this.updateHairCare.bind(this);

    }
    componentDidMount(){
        HairCareService.getHairCareById(this.state.id).then( (res) =>{
            let hairCare = res.data;
            this.setState({nomProduit: hairCare.nomProduit,
                description: hairCare.description,
                prix: hairCare.prix,
                quantite: hairCare.quantite,
 ...
 updateHairCare = (e) => {
      ...
      if (base64Image == !baseImage) 
      {
        ...
        console.log(" est nulle!"); 
          ...
          let hairCare = {nomProduit: this.state.nomProduit,description: this.state.description,prix: this.state.prix, quantite: this.state.quantite,image: this.state.image };
        console.log('hairCare => ' + JSON.stringify(hairCare));
        HairCareService.updateHairCare(hairCare, this.state.id).then( res => {
            this.props.history.push('/hairCare');
        });
    }
        else 
       
        {
            console.log(" il ya une image!"); 
                ...
                HairCareService.uploadImage(image[1]).then(res => {
                // this.props.history.push('/image');
                console.log("**", res);
                let uplaodedImage = res.data;

              let hairCare = {nomProduit: this.state.nomProduit,description: this.state.description,prix: this.state.prix, quantite: this.state.quantite, image: uplaodedImage};
              HairCareService.updateHairCare(hairCare, this.state.id).then( res => {
                  this.props.history.push('/hairCare');
              });
            });
                        ...
                        <div className = "card-body">
                            <form>
                             ...
                             { /**   <div className="form-group">
                                ...
                                <button className="button button1" onClick={this.updateHairCare} >Enregistrer</button>
