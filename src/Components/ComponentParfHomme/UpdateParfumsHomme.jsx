...
class UpdateParfumsComponents extends Component {
    constructor(props){
        ...
        this.updateParfumsHomme = this.updateParfumsHomme.bind(this);

    }
    componentDidMount(){
        ParfumsHommeService.getParfumsHommeById(this.state.id).then( (res) =>{
            let parfumsHomme = res.data;
            this.setState({nomProduit: parfumsHomme.nomProduit,
                description: parfumsHomme.description,
                prix: parfumsHomme.prix,
                quantite: parfumsHomme.quantite,
                image: parfumsHomme.image
             });
        })
    }
 ...
 updateParfumsHomme = (e) => {
      ...
      if (base64Image == !baseImage) 
      {
        ...
        console.log(" est nulle!"); 
          ...
          let parfumsHomme = {nomProduit: this.state.nomProduit,description: this.state.description,prix: this.state.prix, quantite: this.state.quantite,image: this.state.image };
        console.log('parfumsHomme => ' + JSON.stringify(parfumsHomme));
        ParfumsHommeService.updateParfumsHomme(parfumsHomme, this.state.id).then( res => {
            this.props.history.push('/parfumsHomme');
        });
    }
        else 
       
        {
            ...
            ParfumsHommeService.uploadImage(image[1]).then(res => {
                // this.props.history.push('/image');
                console.log("**", res);
                let uplaodedImage = res.data;

              let parfumsHomme = {nomProduit: this.state.nomProduit,description: this.state.description,prix: this.state.prix, quantite: this.state.quantite, image: uplaodedImage};
              ParfumsHommeService.updateParfumsHomme(parfumsHomme, this.state.id).then( res => {
                  this.props.history.push('/parfumsHomme');
              });
            });
                            ...
                            <div className = "card-body">
                                <form>
                                 ...
                                 { /**   <div className="form-group">
                                    ...
                                    <button className="button button1" onClick={this.updateParfumsHomme} >Enregistrer</button>
